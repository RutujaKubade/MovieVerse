...
if (isset($_POST['update'])) {
    ...
    // ---- Handle poster: file upload OR pasted URL (optional - only replace if provided) ----
    $posterClause = '';
    ...
    if (isset($_FILES['poster']) && $_FILES['poster']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            ...
            $posterEsc = $conn->real_escape_string($posterName);
            $posterClause = ", poster='$posterEsc'";
        }
    } elseif (!empty($_POST['poster_url'])) {
        $posterEsc = $conn->real_escape_string(trim($_POST['poster_url']));
        $posterClause = ", poster='$posterEsc'";
    }

    $sql = "UPDATE library
            SET title='$titleEsc', genre='$genreEsc', rating='$ratingEsc', type='$typeEsc', industry='$industryEsc'$posterClause
            WHERE id=$editId";
...
// Fetch existing movie details to pre-fill form
...
<title>Edit Movie | MovieVerse</title>
...
<option value="Book" <?php echo ($movie['type'] === 'Book') ? 'selected' : ''; ?>>📚 Book</option>
</select>
...
<?php $industries = ['Bollywood', 'Hollywood', 'South Indian', 'Other']; ?>
<?php foreach ($industries as $ind): ?>
<option value="<?php echo $ind; ?>" <?php echo ($movie['industry'] === $ind) ? 'selected' : ''; ?>><?php echo $ind; ?></option>
<?php endforeach; ?>
</select>
...
<img src="<?php echo htmlspecialchars(posterSrc($movie['poster'])); ?>"
alt="Current poster"
style="width:100%;max-height:200px;object-fit:cover;border-radius:10px;margin-bottom:10px;">
<?php endif; ?>

<label>Replace Poster (upload a file)</label>
...
<label>OR paste a new poster image URL</label>
